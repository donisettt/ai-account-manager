                ...
                <div class="card-body">
                    <form action="{{ route('tools.update', $tool) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Format: JPG, PNG, GIF, SVG. Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah logo.</small>
                            @error('logo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <div class="mt-2" id="imagePreview" style="display: none;">
                                ...
                                <p class="text-muted small mb-0">Preview logo baru</p>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status_aktif') is-invalid @enderror" 
                                    id="status_aktif" 
                                    name="status_aktif">
                                <option value="1" {{ old('status_aktif', $tool->status_aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status_aktif', $tool->status_aktif) == '0' ? 'selected' : '' }}>Non-Aktif</option>
                            </select>
                            @error('status_aktif')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                      id="keterangan" 
                                      name="keterangan" 
                                      rows="4">{{ old('keterangan', $tool->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
