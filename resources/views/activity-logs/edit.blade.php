...
<div class="py-4">
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('activity-logs.update', $activityLog) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('account_id') is-invalid @enderror" 
                                        ...
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}" {{ old('account_id', $activityLog->account_id) == $account->id ? 'selected' : '' }}>
                                                {{ $account->email }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('tool_id') is-invalid @enderror" 
                                        ...
                                        @foreach($tools as $tool)
                                            <option value="{{ $tool->id }}" {{ old('tool_id', $activityLog->tool_id) == $tool->id ? 'selected' : '' }}>
                                                {{ $tool->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" 
                                           ...
                                           id="waktu" 
                                           name="waktu" 
                                           value="{{ old('waktu', $activityLog->waktu->format('Y-m-d\TH:i')) }}"
                                           required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('aktivitas') is-invalid @enderror" 
                                        ...
                                        @foreach($aktivitasList as $value => $label)
                                            <option value="{{ $value }}" {{ old('aktivitas', $activityLog->aktivitas) == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                      id="keterangan" 
                                      name="keterangan" 
                                      rows="3">{{ old('keterangan', $activityLog->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
